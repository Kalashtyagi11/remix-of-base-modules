...
DECLARE
  ...
  v_n     int;
BEGIN
  SELECT count(*) INTO v_years FROM public.core_fiscal_year f WHERE f.is_active;
  ...
  SELECT count(*) INTO v_open FROM public.core_fiscal_year f
   WHERE f.is_active AND f.planning_open AND f.status <> 'CLOSED';
