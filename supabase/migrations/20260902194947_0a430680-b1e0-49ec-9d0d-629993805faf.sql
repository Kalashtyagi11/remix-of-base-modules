...
DECLARE
  ...
  v_id_changed BOOLEAN; v_text_changed BOOLEAN;
BEGIN
  -- ---------------- Audit / Engagement Type ----------------
  v_id_changed   := (TG_OP = 'INSERT') OR (NEW.engagement_type_id IS DISTINCT FROM OLD.engagement_type_id);
  v_text_changed := (TG_OP = 'INSERT') OR (NEW.engagement_type    IS DISTINCT FROM OLD.engagement_type);

  IF v_id_changed OR v_text_changed THEN
    IF v_id_changed AND NEW.engagement_type_id IS NOT NULL THEN
      ...
      IF v_text_changed AND NEW.engagement_type IS NOT NULL
         AND btrim(NEW.engagement_type) <> '' AND btrim(NEW.engagement_type) <> v_name THEN
        RAISE EXCEPTION 'IA_REFERENCE_TEXT_CONFLICT: "%" does not match the selected AUDIT_TYPE "%"',
          NEW.engagement_type, v_name;
      END IF;
      NEW.engagement_type := v_name;
    ELSIF v_text_changed AND NEW.engagement_type IS NOT NULL AND btrim(NEW.engagement_type) <> '' THEN
      -- reclassification by canonical display name (governed resolve)
      ...
      NEW.engagement_type := v_name;
    ELSIF NEW.engagement_type_id IS NOT NULL THEN
      PERFORM public.ia_reference_assert_id('AUDIT_TYPE', NEW.engagement_type_id);
      SELECT name INTO v_name FROM public.ia_reference_value WHERE id = NEW.engagement_type_id;
      NEW.engagement_type := v_name;
    END IF;
  ...
  -- ---------------- Coverage Category ----------------
  v_id_changed   := (TG_OP = 'INSERT') OR (NEW.coverage_category_id IS DISTINCT FROM OLD.coverage_category_id);
  v_text_changed := (TG_OP = 'INSERT') OR (NEW.coverage_category    IS DISTINCT FROM OLD.coverage_category);

  IF v_id_changed OR v_text_changed THEN
    IF v_text_changed AND NEW.coverage_category IS NOT NULL
       AND btrim(NEW.coverage_category) <> ''
       AND public.ia_is_risk_band_label(NEW.coverage_category) THEN
      RAISE EXCEPTION 'IA_INVALID_REFERENCE_SEMANTICS: "%" is a risk classification, not a coverage category',
        NEW.coverage_category;
    END IF;

    IF v_id_changed AND NEW.coverage_category_id IS NOT NULL THEN
      ...
      IF v_text_changed AND NEW.coverage_category IS NOT NULL
         AND btrim(NEW.coverage_category) <> '' AND btrim(NEW.coverage_category) <> v_name THEN
        RAISE EXCEPTION 'IA_REFERENCE_TEXT_CONFLICT: "%" does not match the selected COVERAGE_CATEGORY "%"',
          NEW.coverage_category, v_name;
      END IF;
      NEW.coverage_category := v_name;
    ELSIF v_text_changed AND NEW.coverage_category IS NOT NULL AND btrim(NEW.coverage_category) <> '' THEN
      ...
      SELECT name INTO v_name FROM public.ia_reference_value WHERE id = v_id;
      NEW.coverage_category := v_name;
    ELSIF NEW.coverage_category_id IS NOT NULL THEN
      PERFORM public.ia_reference_assert_id('COVERAGE_CATEGORY', NEW.coverage_category_id);
      SELECT name INTO v_name FROM public.ia_reference_value WHERE id = NEW.coverage_category_id;
      NEW.coverage_category := v_name;
    END IF;
...
DECLARE
  ...
  v_id_changed BOOLEAN; v_text_changed BOOLEAN;
BEGIN
  v_id_changed   := (TG_OP = 'INSERT') OR (NEW.follow_up_type_id IS DISTINCT FROM OLD.follow_up_type_id);
  v_text_changed := (TG_OP = 'INSERT') OR (NEW.follow_up_type    IS DISTINCT FROM OLD.follow_up_type);

  IF v_id_changed OR v_text_changed THEN
    IF v_id_changed AND NEW.follow_up_type_id IS NOT NULL THEN
      ...
      IF v_text_changed AND NEW.follow_up_type IS NOT NULL
         AND btrim(NEW.follow_up_type) <> '' AND btrim(NEW.follow_up_type) <> v_name THEN
        RAISE EXCEPTION 'IA_REFERENCE_TEXT_CONFLICT: "%" does not match the selected FOLLOW_UP_TYPE "%"',
          NEW.follow_up_type, v_name;
      END IF;
      NEW.follow_up_type := v_name;
    ELSIF v_text_changed AND NEW.follow_up_type IS NOT NULL AND btrim(NEW.follow_up_type) <> '' THEN
      ...
      SELECT name INTO v_name FROM public.ia_reference_value WHERE id = v_id;
      NEW.follow_up_type := v_name;
    ELSIF NEW.follow_up_type_id IS NOT NULL THEN
      PERFORM public.ia_reference_assert_id('FOLLOW_UP_TYPE', NEW.follow_up_type_id);
      SELECT name INTO v_name FROM public.ia_reference_value WHERE id = NEW.follow_up_type_id;
      NEW.follow_up_type := v_name;
    END IF;
