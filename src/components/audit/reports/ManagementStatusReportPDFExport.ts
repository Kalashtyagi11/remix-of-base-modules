 ...
 * recomputed here: the PDF is a presentation of the server-side status engine.
 *
 * NO-HARDCODING RULE: the section list, section order, headings, page breaks
 * and KPI columns come from the resolved report definition; branding comes from
 * the Document Foundation. Nothing organisation-specific lives in this file.
 */
...
export interface ManagementStatusPdfSection {
  sectionKey: string;
  heading: string;
...
export interface ManagementStatusPdfMetric {
  metricCode: string;
  label: string;
...
export interface ManagementStatusPdfMeta {
  ...
  /** Resolved report-definition sections, in configured order. */
  sections?: ManagementStatusPdfSection[];
  /** Resolved metric registry entries for the KPI section. */
  metrics?: ManagementStatusPdfMetric[];
  /** Branding resolved from the Document Foundation. */
...
function metricValue(data: any, path: string | null | undefined, formatter?: string | null): string {
  if (!path) return '—';
  const raw = path.split('.').reduce<any>((acc, key) => (acc == null ? undefined : acc[key]), data);
  if (raw === null || raw === undefined) return '—';
  if (formatter === 'percent') return `${raw}%`;
  if (formatter === 'hours') return `${raw} h`;
  return String(raw);
}
...
export function buildManagementStatusPdf(
  ...
  const branding: ExportBranding = meta.branding ?? DEFAULT_AUDIT_BRANDING;
  const pw = doc.internal.pageSize.getWidth();
  const margin = 14;
  const k = data.kpis || {};
  const provenance: Record<string, any> = (data as any).provenance ?? {};

  /** Fallback order is used only when no report definition is supplied. */
  const sections: ManagementStatusPdfSection[] =
    meta.sections && meta.sections.length
      ? meta.sections
      : [
          { sectionKey: 'MSR_COVER', heading: 'Cover & reporting context' },
          { sectionKey: 'MSR_HEALTH', heading: 'Executive Summary & Plan Health', startOnNewPage: true },
          { sectionKey: 'MSR_KPI', heading: 'Key performance indicators' },
          { sectionKey: 'MSR_PERIOD_MOVEMENT', heading: 'Reporting Period Activity', startOnNewPage: true },
          { sectionKey: 'MSR_COMPLETED_AUDITS', heading: 'Audits Completed — Conclusions & Material Issues', startOnNewPage: true },
          { sectionKey: 'MSR_THEMES', heading: 'Recurring Themes, Assurance Coverage & Outlook', startOnNewPage: true },
          { sectionKey: 'MSR_ENGAGEMENTS', heading: 'Engagement Status', startOnNewPage: true },
          { sectionKey: 'MSR_FINDINGS', heading: 'Findings, Corrective Actions, Prior Issues & Capacity', startOnNewPage: true },
          { sectionKey: 'MSR_ATTENTION', heading: 'Management Attention & Decisions Required', startOnNewPage: true },
          { sectionKey: 'MSR_PROVENANCE', heading: 'Basis of Preparation', startOnNewPage: true },
        ];

  let started = false;
  function beginSection(section: ManagementStatusPdfSection): number {
    if (started && section.startOnNewPage !== false) doc.addPage();
    started = true;
    return renderSectionHeading(doc, branding, section.heading, 18) ?? 30;
  }

  const renderers: Record<string, (section: ManagementStatusPdfSection) => void> = {
    MSR_COVER: () => {
      renderCoverPage(doc, branding, {
        ...
        metadata: [
          ...
          { label: 'Report Mode', value: data.report_mode || provenance.report_definition_code || '—' },
          {
    ...
    MSR_HEALTH: (section) => {
      ...
      doc.text(doc.splitTextToSize(summary, pw - margin * 2), margin, y0 + 4);
      let y = y0 + 26;
      doc.setFontSize(8);
      doc.setTextColor(110, 110, 110);
      doc.text(`Health basis: ${data.health?.basis ?? ''}`, margin, y);
      const fired = data.health?.rules_triggered ?? [];
      if (fired.length) {
        autoTable(doc, {
          startY: y + 6,
          head: [['Health rule triggered', 'Metric', 'Observed', 'Configured threshold', 'Severity', 'Score']],
          body: fired.map((r) => [
            r.label, r.metric, String(r.observed), String(r.threshold), r.severity ?? '—', String(r.score),
          ]),
          theme: 'grid',
    ...
    MSR_KPI: (section) => {
      ...
      const y = started ? undefined : beginSection(section);
      autoTable(doc, {
        startY: y ?? ((doc as any).lastAutoTable?.finalY ?? 30) + 8,
        head: [['Executive KPI', 'Value']],
        body: metrics.length
          ? metrics.map((m) => [m.label, metricValue(data, m.sourcePath, m.formatter)])
          : [['No metrics enabled', '—']],
        theme: 'grid',
